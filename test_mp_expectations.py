"""
    Script that tests the pen-and-paper derivations of the expectations from the MP-VAE
"""
...
@njit(parallel=True)
def sample_bernoulli_cross_term(result_array, p, h1, h2, random_generator):
    nlags = len(h1)
    for i in prange(len(result_array)):
        ...
        for j in range(len(p) - nlags + 1):
            ysum += h1 @ x[j:nlags+j][::-1] * (h2 @ (x[j:nlags+j][::-1])**2)
        result_array[i] = ysum

@njit(parallel=True)
def sample_normal_cross_term(result_array, mu, var, h1, h2, random_generator):
    nlags = len(h1)
    for i in prange(len(result_array)):
...
@njit(parallel=True)
def sample_bernoulli_square2_term(result_array, p, h2, random_generator):
    nlags = len(h2)
    for i in prange(len(result_array)):
        x = random_generator.uniform(size=len(p)) <= p
        x = x.astype(np.float64)
        ysum = 0.0
        for j in range(len(p) - nlags + 1):
            ysum += (h2 @ (x[j:nlags+j][::-1])**2)**2
        result_array[i] = ysum

@njit(parallel=True)
def sample_normal_square2_term(result_array, mu, var, h2, random_generator):
    nlags = len(h2)
    for i in prange(len(result_array)):
        x = np.sqrt(var) * random_generator.standard_normal(size=len(mu)) + mu
        ysum = 0.0
...
if __name__ == "__main__":
    ...
    for s, seed in enumerate(seeds):
        ...
        # Generate random FIR filters
        h1 = random_obj.uniform(size=KERNEL_MEMORY)
        h2 = random_obj.uniform(size=KERNEL_MEMORY)
        ...
        if x_distribution == 'bernoulli':
            sample_bernoulli_cross_term(empirical, p, h1, h2, random_obj)
        elif x_distribution == 'normal':
            sample_normal_cross_term(empirical, mu, variance, h1, h2, random_obj)
        else:
            raise ValueError
        ...
        derived = np.sum(np.convolve(ex2, h2, 'valid') * np.convolve(ex, h1, 'valid') + np.convolve(ex3 - ex2 * ex, h1 * h2, 'valid'))
        ...
        if x_distribution == 'bernoulli':
            sample_bernoulli_square2_term(empirical, p, h2, random_obj)
        elif x_distribution == 'normal':
            sample_normal_square2_term(empirical, mu, variance, h2, random_obj)
        else:
            raise ValueError
        ...
        # Calculate expectation from derived formula
        derived = np.sum(np.convolve(ex2, h2, 'valid')**2 + np.convolve(ex4 - ex2**2, h2**2, 'valid'))
        print(f"Theoretial expectation: {derived}")
